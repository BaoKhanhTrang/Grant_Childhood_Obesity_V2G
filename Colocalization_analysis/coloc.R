...
.libPaths( c( .libPaths(), "/home/trangk/R/x86_64-pc-linux-gnu-library/4.2") )
library(coloc)
library(data.table)
library(rjson)
library(tidyverse)
...
trait_region = fread(file=traitFilePath, sep="\t", header=TRUE,fill=TRUE)
...
for(i in 1:length(trait_chrom_pos)){
        lead_SNP_pos_tabix_with_chr = paste0(gsub("_",":",trait_chrom_pos[[i]]), "-", gsub("^.*?_","",trait_chrom_pos[[i]]))
        lead_SNP_pos_tabix_without_chr = paste0(gsub("chr", "", gsub("_",":",trait_chrom_pos[[i]])), "-", gsub("^.*?_","",trait_chrom_pos[[i]]))
...
if(!file.exists(paste(out_prefix,"coloc_results_pp4_cond.txt",sep="_"))){
    ...
    eGeneTissue_region = unique(eGeneTissue_region)
    eGeneTissue_region = eGeneTissue_region[order(eGeneTissue_region$SNP),]
    if(length(which(duplicated(eGeneTissue_region$SNP)))>0){
            eGeneTissue_region = eGeneTissue_region[-which(duplicated(eGeneTissue_region$SNP)),]
    }
